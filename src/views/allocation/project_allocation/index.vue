    ...
    <div class="allo_container">
        <div class="left">
            <div class="search">
                ...
                <div class="radioBox">
                    <el-radio-group v-model="allocat">
                        <span class="fili">
                            <el-radio :label="0" style="margin-bottom:7px;" @click.native.prevent="clickfull(0)">allocated</el-radio>
                            <el-radio :label="1" style="margin-bottom:7px;" @click.native.prevent="clickfull(1)">not be allocated</el-radio>
                        </span>
                    </el-radio-group>
                </div>
            </div>
            <div class="table">
                <perfect-scrollbar class="background-container">
                    <div class="box-card" v-for="(group,index) in groupList" :key="index" @click="getProjectID(group.projectId); setCenter(index) " :class="{selectedColor:selectIndex === index}">
                        <div class="top">
                            <p class="tit">Group {{group.grid}}</p>
                            <p class="t_name">pre-result: CS{{group.projectId}}</p>
                        ...
                        <div class="pre">
                            <el-button size="mini" class="pdB10" style="margin-left:10px" @click.stop="getProjectID(group.pre1); getResult(index,group.pre1) ">pre1: CS{{group.pre1}}</el-button>
                            <el-button size="mini" class="pdB10" @click.stop="getProjectID(group.pre2); getResult(index,group.pre2)">pre2: CS{{group.pre2}}</el-button>
                            <el-button size="mini" class="pdB10" @click.stop="getProjectID(group.pre3); getResult(index,group.pre3)">pre3: CS{{group.pre3}}</el-button>
                        </div>
                        <div class="allo">
                            ...
                            <el-button size="small" @click.stop="submit(group.id,group.result)" style="margin-bottom:8px;">Submit</el-button>
...
export default {
    ...
    watch: {
        ...
        'allocat': function (newVal, oldVal) {
            this.findGroup()
        },
    ...
    methods: {
        ...
        findGroup() {
            ...
            let num = this.allocat

            allocation.getValidGroup(keyword, num).then(response => {
                console.log(response)
                ...
                for (var i in this.groupList) {
                    if(this.groupList[i].finalId!==null && this.groupList[i].finalId!=='0'){
                        ...
                        this.$set(this.groupList[i], 'result', this.groupList[i].projectId)
                    }
        ...
        getAllValidGroup() {
            allocation.getAllValidGroup().then(response => {
                console.log(response)
                this.groupList = response.data.validGroup
